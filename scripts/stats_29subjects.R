...
## RUN GLMERS FOR ANALYSIS #1
	# Full model
supsw_full.glmer = glmer(Correct ~ Lg_HeardContrast * Main_ContextContrast + (0+Lg_HeardContrast*Main_ContextContrast|Subject) + (1|Stimuli), family="binomial", data=supsw)

summary(supsw_full.glmer)

	# Test for language effect
supsw_nolg.glmer = glmer(Correct ~ Lg_HeardContrast * Main_ContextContrast - Lg_HeardContrast + (0+Lg_HeardContrast*Main_ContextContrast|Subject) + (1|Stimuli), family="binomial", data=supsw)

anova(supsw_full.glmer, supsw_nolg.glmer)

	# Test for context effect
supsw_nocontext.glmer = glmer(Correct ~ Lg_HeardContrast * Main_ContextContrast - Main_ContextContrast + (0+Lg_HeardContrast*Main_ContextContrast|Subject) + (1|Stimuli), family="binomial", data=supsw)

anova(supsw_full.glmer, supsw_nocontext.glmer)

	# Test for interaction of language and context
supsw_nolgxcontext.glmer = glmer(Correct ~ Lg_HeardContrast * Main_ContextContrast - Lg_HeardContrast:Main_ContextContrast + (0+Lg_HeardContrast*Main_ContextContrast|Subject) + (1|Stimuli), family="binomial", data=supsw)
...
## RUN GLMERS FOR ANALYSIS #2
	# Full model
supsw_resp_full.glmer = glmer(ResponseContrast ~ Lg_HeardContrast * Main_ContextContrast + (1+Lg_HeardContrast*Main_ContextContrast|Subject) + (1|Stimuli), family="binomial", data=supsw)

summary(supsw_resp_full.glmer)

	# Test for language effect
supsw_resp_nolg.glmer = glmer(ResponseContrast ~ Lg_HeardContrast * Main_ContextContrast - Lg_HeardContrast + (1+Lg_HeardContrast*Main_ContextContrast|Subject) + (1|Stimuli), family="binomial", data=supsw)

anova(supsw_resp_full.glmer, supsw_resp_nolg.glmer)

	# Test for context effect
supsw_resp_nocontext.glmer = glmer(ResponseContrast ~ Lg_HeardContrast * Main_ContextContrast - Main_ContextContrast + (1+Lg_HeardContrast*Main_ContextContrast|Subject) + (1|Stimuli), family="binomial", data=supsw)

anova(supsw_resp_full.glmer, supsw_resp_nocontext.glmer)

	# Test for interaction of language and context
supsw_resp_nolgxcontext.glmer = glmer(ResponseContrast ~ Lg_HeardContrast * Main_ContextContrast - Lg_HeardContrast:Main_ContextContrast + (1+Lg_HeardContrast*Main_ContextContrast|Subject) + (1|Stimuli), family="binomial", data=supsw)
