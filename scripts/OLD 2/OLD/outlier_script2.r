...
d <- read.table( 'all_all.txt' , sep = '\t', header = T) 

# create the linear model
l <- lm( RT ~ Subject*Response*Main_Context*Lg_Heard,data=d) 
		...
		for(main_context in main_contexts){
			for(lg_heard in lg_heards){
				...
				if(n > 0){
					...
					parameters <- data.frame(Subject, Response, Main_Context, Lg_Heard)
